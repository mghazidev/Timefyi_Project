...
import TFloatingPomodoro from "../shared/TFlowtingPomodoro";
  ...
  return (
    ...
    >
      <div className="flex gap-2 justify-end text-zinc-500">
        <button onClick={() => setShowMini(true)} className="cursor-pointer">
        ...
        <button onClick={handleToggleFullscreen} className="cursor-pointer">
          {isFullscreen ? (
            <SmallScreenIcon size={16} className="hover:text-zinc-200" />
          ) : (
            <FullscreenIcon size={16} className="hover:text-zinc-200" />
          )}
        </button>
      </div>
      ...
      <TFloatingPomodoro
        ...
        minutes={Math.floor((totalDuration - elapsed) / 60)}
        seconds={(totalDuration - elapsed) % 60}
        isRunning={isRunning}
